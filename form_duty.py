...
0. Просмотреть ToDo
1. Заполнить шаблон в файле "Исходный_Шаблон_Весь_период.xlsx"
1.1 На листе "График"
    - Заполнить версию на графике
    - Заполнить количество участников на каждый день
    - Заполнить кто, когда дежурит (см. согласование скрипта в части стартовых ячеек !!!)
    ...
    - Если требуется, то добавить по аналогии новые блюда
1.2 На листе "Шаблон"
    ...
    - Если требуется поменять местами каши с мясом с кашами с молоком, то заменить: "1 Завтрак" на "5 Ужин", при этом остальные ячейки не перемещать
    - Добавить или удалить рассчет перловки, как каши, добавляемые к какой-то другой базовой
    - Заменить дату выезда
    - Творчески подойти к редактированию данной страницы, так, например, 
        -- так проверить, что корректно указаны продукты, употребляемые на завтрак, обед и ужин.
...
#  3. Добавить в даты в шапке таблицы дежурств населенные пункты или знаковые места, которые предполагается проезжать
#  - 4. Добавлять имена участников на листы раскладок в фиксированную ячейку
#  5. Если выпадают макароны с сыром, то изюм не нужен, и не нужен будетрброд с сыром, поправить
...
MEAL_PLACE = {
    "Геркулес": "G8", "Пшенка": "I8", "Макароны(сыр)": "K8",
    "Харчо": "G14", "Борщ": "I14", "Гороховый": "K14",
    "Гречка": "G20", "Рис": "I20", "Макароны(мясо)": "K20", "Без супа": "M14",
}
...
def form_duty(patten_file, out_file_name):
    ...
    START_X = get_cell_x_num_by_letter('F')   # X-координата левого верхнего угла расписания, с которого начинаются фамилии дежурных
    START_Y = 8                               # Y-координата левого верхнего угла расписания
    ...
    for p in persons:
        ...
        target.title = p
        target["I2"] = p   # Пишем имя на листе
        persons_meals[p] = {}
        for ft in data_base:
            if ft.person == p:
                try:
...
if __name__ == "__main__":
    ...
    form_duty(patten_file="Исходный_Шаблон_Весь_период.xlsx", out_file_name="Раскладка_Питание_2026.08.22_Весь_период.xlsx")
